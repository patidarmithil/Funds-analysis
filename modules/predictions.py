"""
predictions.py — Robust prediction models for NAV forecasting.

All models:
  1. Forecast FORWARD from the last known NAV.
  2. Use log-returns and trend features rather than raw NAV (prevents collapse).
  3. Return a DataFrame with ds, yhat, yhat_lower, yhat_upper for future dates only.
  4. Are evaluated on a hold-out validation set.
"""
import warnings
import numpy as np
import pandas as pd
...
def _future_dates(df: pd.DataFrame, periods: int) -> pd.DatetimeIndex:
    """Generate business-day future dates starting after the last known date."""
...
def _build_result(future_ds, yhat: np.ndarray, ci_pct: float = 0.05) -> pd.DataFrame:
    """Build standardised forecast DataFrame."""
    spread = np.std(yhat) * ci_pct + yhat[-1] * 0.01  # minimum spread = 1% of last NAV
    return pd.DataFrame({
...
def _log_return_features(series: pd.Series, n_lags: int = 10) -> tuple:
    """
    Build supervised dataset from LOG RETURNS (not raw NAV).
    This prevents scale drift and oscillation in recursive forecasting.
    """
...
def _forecast_from_returns(model, last_nav: float, last_returns: np.ndarray,
    ...
    """
    Recursively forecast by predicting log-returns then cumulating to NAV.
    Much more stable than predicting raw NAV directly.
    """
    hist = list(last_returns[-60:])   # keep a rolling window of recent returns
    nav  = last_nav
    navs = []

    for _ in range(periods):
        lag_feats = hist[-n_lags:][::-1]           # most recent lags first
        ...
        # Clip extreme predictions to prevent explosion
...
def _run_prophet(train: pd.DataFrame, periods: int) -> pd.DataFrame:
    ...
    # Clip any negative forecasts (NAV can't be negative)
...
def _run_holt_winters(train: pd.DataFrame, periods: int) -> pd.DataFrame:
    ...
    try:
        m = ExponentialSmoothing(
            series, trend='add', seasonal='add' if season >= 4 else None,
    ...
    spread    = np.abs(fc) * 0.04   # ±4% confidence band

    return pd.DataFrame({
        'ds':         future_ds,
        'yhat':       fc,
        'yhat_lower': fc - spread,
        'yhat_upper': fc + spread,
    })


def _run_sklearn_model(train: pd.DataFrame, periods: int,
                        model_cls, **kwargs) -> pd.DataFrame:
    """
    Predict log-returns, then compound back to NAV.
    This produces stable, trend-following forecasts instead of oscillating ones.
    """
    n_lags   = 10
    series   = train['y']
...
def run_model(df: pd.DataFrame, model_name: str,
              future_periods: int = 90) -> tuple[pd.DataFrame, dict]:
    """
    1. Split df into train/test (80/20).
    2. Train model on train, forecast n_test steps, evaluate vs test.
    3. Train model on FULL df, forecast future_periods steps.
    4. Return (future_forecast_df, metrics).
    """
...
def run_ensemble(df: pd.DataFrame,
                 ...
                 future_periods: int = 90,
                 progress_cb=None) -> dict:
    """Auto-optimised ensemble via SLSQP on validation RMSE."""
    n_test  = max(30, int(len(df) * 0.20))
    train   = df.iloc[:-n_test].copy().reset_index(drop=True)
    test    = df.iloc[-n_test:].copy().reset_index(drop=True)
    ...
    # Ensemble future forecast — weighted average of NAV arrays
    if full_forecasts:
        ...
        ens_forecast = pd.DataFrame({
            'ds':         future_ds,
            'yhat':       ens_yhat,
            'yhat_lower': ens_lower,
            'yhat_upper': ens_upper,
        })
    else:
        ens_forecast = pd.DataFrame()
