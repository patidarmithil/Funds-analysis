...
def calculate_sharpe(df: pd.DataFrame, rf: float = RISK_FREE_RATE) -> float:
    """Sharpe Ratio = (Rp - Rf) / σp  [annualised]."""
    r = df['returns'].dropna()
    if r.std() == 0:
        return np.nan
...
def calculate_sortino(df: pd.DataFrame, rf: float = RISK_FREE_RATE) -> float:
    """Sortino Ratio = (Rp - Rf) / downside σ  [annualised]."""
    r = df['returns'].dropna()
    daily_rf  = (1 + rf) ** (1 / TRADING_DAYS) - 1
    excess    = r - daily_rf
...
def calculate_volatility(df: pd.DataFrame) -> float:
    """Annualised volatility (std of daily returns)."""
...
def calculate_alpha_beta(fund_ret: pd.Series, bm_ret: pd.Series,
    ...
    Returns alpha (annualised), beta, r_squared.
    """
    daily_rf = (1 + rf) ** (1 / TRADING_DAYS) - 1
...
def calculate_correlation_matrix(all_funds: dict) -> pd.DataFrame:
    """Build return correlation matrix across all loaded funds."""
    series = {}
...
def get_fund_summary(df: pd.DataFrame) -> dict:
    """One-row summary dict for the Overview table."""
    return dict(
